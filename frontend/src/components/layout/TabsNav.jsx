...
export default function TabsNav({ current, onChange, player }) {
  ...
  return (
    <nav className="relative z-10 max-w-[1600px] mx-auto w-full px-4 md:px-6 pt-[calc(env(safe-area-inset-top)+12px)] md:pt-5" data-testid="tabs-nav">
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button key={tab.key} onClick={() => onChange(tab.key)}
                className={`px-2.5 py-2.5 rounded-full flex items-center gap-1.5 transition-all font-display text-[10px] tracking-[0.2em] font-semibold uppercase ${
      ...
      <div className="hidden md:flex flex-col items-center md:flex-row md:items-center md:justify-between flex-wrap gap-3 md:gap-4 mb-3 md:mb-4">
        ...
        <div className="flex items-center gap-2 md:gap-3 flex-wrap justify-center md:justify-end">
          ...
          <div className="glass-panel rounded-full p-1 flex gap-1 flex-nowrap overflow-x-auto hide-scrollbar justify-center md:justify-start md:flex-wrap md:overflow-visible">
            {TABS.map((tab) => {
              ...
              return (
                <button key={tab.key} onClick={() => onChange(tab.key)}
                  className={`px-2 md:px-3 py-1.5 md:py-2 rounded-full flex items-center gap-1.5 transition-all font-display text-[10px] tracking-[0.2em] font-semibold uppercase ${
                    active ? 'bg-[rgba(0,229,255,0.12)] text-[#00E5FF] shadow-[0_0_16px_rgba(0,229,255,0.35)]' : 'text-slate-400 hover:text-slate-200'
                  }`}
                  data-testid={`tab-${tab.key}`}
                >
